...
//send a custom message by saying someone user name or saying a specific word (can be done multiple time, consult Examples.js)
  if (message.startsWith(`Oyun`)) {
     bot.message.send(`Oyunumuzu kısaca anlatmak gerekirse 6 kişi ile oynanmaktadır herkes sıra olduğunda ilk sıradan başlayıp 1.sırada ki kişi için zar çevirilir çıkan sayıya göre ilerlemektedir ve bastığı renklerin de belli bir amacı olmaktadır`);
     bot.message.send(`Renklerin ne anlama geldiğini öğrenmek istiyorsanız yönetici alanında ki duvara bakınız, iyi oyunlar dilerim !`);
  };


  if (message.startsWith(`Sponsor`)) {
   bot.message.send(`Oyuna sponsor olup isminizi yazdırmak isterseniz @Atekinz ile iletişime geçin.`);
  };
...
bot.on('playerJoin', (user) => {
  ...
  bot.message.send(`@${user.username} Monopoly Oyun odamıza hoş geldiniz!`);
});
